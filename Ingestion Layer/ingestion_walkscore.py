...
def main():
    apiKey='039af3658b84fcc97829ec246db6bab2'
    walkscore = WalkScore(apiKey)
    count=0
    print('works')
    cursor = db.restaurants.find({})
    db.restaurants_yelp_walkscore.drop()
    for document in cursor:
        address="".join(document["location"]["display_address"])
        lat=document["coordinates"]["latitude"]
        lon=document["coordinates"]["longitude"]
        try:
            score=walkscore.makeRequest(address, lat, lon)
            walk_score=score['walkscore']
        except:
            walk_score=101
        document["walk_score"]=walk_score
        db.restaurants_yelp_walkscore.insert_one(document)
        count+=1
        print(count)
